...
<table class="ntable">
	...
	{if[{var=CAN_WRITE_GLOBAL_MAILS} || {var=founder} == Core::getUser()->get("userid")]}<tr>
		<td colspan="2" class="center">{link[WRITE_GLOBAL_MAIL]}"game.php/Alliance/GlobalMail"{/link}</td>
	</tr>{/if}
	<tr>
		<td colspan="2" class="center">{@textextern}</td>
	</tr>
	...
	<tr>
		<td colspan="2" class="center">{@textintern}</td>
	</tr>{/if}
	...
	{if[!Core::getUser()->get("aid") && !{var=appInProgress}]}<tr>
		<td colspan="2" class="center"><input name="aid" type="hidden" value="{@aid}" /><input name="enter" type="submit" value="{lang}JOIN{/lang}" class="button" /></td>
	</tr>{/if}
