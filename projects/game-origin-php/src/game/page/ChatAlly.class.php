...
class ChatAlly extends Page
{
	...
	protected function index()
	{
		...
		if( $u_id == "" )
		{
			$redir_url = socialUrl(RELATIVE_URL . 'game.php/Chat');
			/* if( defined('SN') )
			{
				$redir_url .= '';
			} */
		...
		$messages = array();
		$result = sqlSelect("chat2ally c", array("c.time", "c.message", "u.username"), "LEFT JOIN ".PREFIX."user u ON (u.userid = c.userid)", "c.allyid = ".sqlVal($u_id), "messageid DESC", "75");
	...
	protected function sendMessage($message)
	{
		...
		if($message != "" && $u_id != 0 && !$this->checkRO() )
		{
			...
			$mes = sqlSelectRow("chat2ally", array("messageid", "message"), "", "userid = ".sqlVal($userid), "messageid DESC", "1");
			if($message != $mes["message"])
			{
				Core::getQuery()->insert("chat2ally", array("time", "userid", "allyid", "message"), array(time(), $userid, $u_id, $message));
			}
		}
		return $this->index();
