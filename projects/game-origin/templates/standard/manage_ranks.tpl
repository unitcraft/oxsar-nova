...
<table class="ntable">
	...
	<tbody>{while[ranks]}<tr>
		...
		<td class="center"><input type="image" src="{const=RELATIVE_URL}images/error2.gif" name="delete_{loop}rankid{/loop}" title="{lang}REMOVE{/lang}" alt="{lang}REMOVE{/lang}" /></td>
	</tr>{/while}</tbody>
	{else}<tr>
