...
USER_TABLES=(
  ...
  na_res_log na_res_transfer na_event_src
)
...
# Группа 2: таблицы где фильтр по planetid IN (...)
PLANET_TABLES=(
  na_building2planet na_galaxy na_unit2shipyard
...
for tbl in "${PLANET_TABLES[@]}"; do
  ...
  docker exec "$LEGACY_CONTAINER" mysqldump -u"$LEGACY_USER" -p"$LEGACY_PASS" "$LEGACY_DB" \
    --no-create-info --skip-extended-insert --skip-comments --hex-blob \
    --where="planetid IN ($PLANETIDS) OR moonid IN ($PLANETIDS)" "$tbl" 2>/dev/null >> "$OUT" || true
  echo "" >> "$OUT"
done
