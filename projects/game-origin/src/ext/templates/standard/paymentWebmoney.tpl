...
function convCredit()
{
	...
	var purse_t = document.getElementById('purse_t')[document.getElementById('purse_t').selectedIndex].value;
	if(isInteger(el_value) == false){
		...
		if(purse_t == 'WMZ')
		{
			to_pay = Math.max( Math.pow(el_value, rootModifier) * z_change, z_min);
			if(to_pay > z_max)
			{
				to_pay = z_max;
				el_value = z_max / z_change;
		...
		{
			to_pay = Math.max( Math.pow(el_value, rootModifier) * e_change, e_min);
			if(to_pay > e_max)
			{
				to_pay = e_max;
				el_value = e_max / e_change;
				el_value = Math.pow(el_value, powModifier);
				el_value = Math.round(el_value);
				el.value = el_value;
			}
...
<form method="post" action="{@wm_process_url}">
...
<table class="ntable">
	...
	<tr>
      ...
      {lang=PAY_MAIN_TEXT}
      <!-- <p />
      Максимальная сумма пополнения в валюте webmoney составляет:
      <nobr>WMZ : WMR : WME = {@z_max} : {@r_max} : {@e_max}</nobr> -->
    </td>
	</tr>
</table>
<table class="ntable">
	...
	<tr>
		<td class="right" width="50%">Тип кошелька:</td>
    <td class="left" width="50%">
      <select name="purse_t" id="purse_t" onChange="convCredit();">
	...
	<tr>
		<td colspan="2" class="center">Курс: WMR : WMZ : WME = {@r_change} : {@z_change} : {@e_change}</td>
	</tr>
	<tr>
