...
class EventHandler
{
	...
	*/
	...
	protected function queryExpiredEvents($cur_time, $max_batch_time = 10, $limit = 5)
	{
		Core::getQuery()->update("events",
			...
			"(time <= ".sqlVal($cur_time)." and processed='".EVENT_PROCESSED_WAIT."' and prev_rc IS NULL)"
			);

		$select = array("e.eventid", "e.mode", "e.start", "e.time", "e.planetid", "e.destination",
			"e.data", "e.user AS userid", "e.parent_eventid", "e.artid",
			"u.points", "d.userid AS destuser", "u.username", "d.username AS destname", "p.planetname", "p2.planetname AS destplanet");
		...
		$result = sqlSelect("events e", $select, $joins,
			"e.time = ".sqlVal($cur_time)." and processed=".EVENT_PROCESSED_WAIT
	...
	*/
	...
	protected function build($row, $data)
	{
		...
		Achievements::processAchievements($row["userid"], $row["planetid"]);
	...
	protected function position($row, $data)
	{
		...
		*/
		...
		Achievements::processAchievements($row["userid"], $row["destination"]);
	...
	{
		error_log('In allianceAttack of base EventHandler', 'error');
		$this->removeEvent($row["eventid"], true);
		return $this;
	}
