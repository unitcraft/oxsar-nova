...
* @version $Id: LanguageExporter.class.php 23 2010-04-03 19:08:34Z craft $
*/
...
class LanguageExporter
{
  ...
  */
  public function setDestination($destination)
  {
    ...
    {
      throw new GenericException("Export destination folder does not exist or is not writable.");
    }
  ...
  */
  public function setFile($filename)
  {
    ...
    {
      throw new GenericException("Export destination file already exists.");
    }
    $this->file = $filename;
    return $this;
  }
  ...
  */
  public function startExport()
  {
    ...
    $data .= '<lang>'.$this->n;
    $where = is_null($this->langId) ? "" : "languageid = ".sqlVal($this->langId)." OR langcode = ".sqlVal($this->langId);
    ...
    while($row = Core::getDB()->fetch($result))
    {
      ...
      foreach($groups as $group)
      {
        ...
        $_result = Core::getQuery()->select("phrases", array("title", "content"), "", "languageid = ".sqlVal($row["languageid"])." AND phrasegroupid = ".sqlVal($group["phrasegroupid"]), "title ASC");
