...
* @version $Id: LanguageImporter.class.php 23 2010-04-03 19:08:34Z craft $
*/
...
//      catch(Exception $e) { $e->printError(); }
    ...
    {
      throw new GenericException("Unkown data supplyed for importer.");
    }
...
//    catch(Exception $e) { $e->printError(); }
  ...
  */
  protected function import()
  {
    ...
    {
      throw new GenericException("Supplied data for importer is not an array.");
    }
  ...
  */
  protected function getDataFromFile($file)
  {
    if(!file_exists($file))
    {
      throw new GenericException("Could not found import file. Make sure the path is correct or try absolute path. Given path: ".$file);
    }
  ...
  */
  ...
  {
    $result = Core::getQuery()->select("languages", array("languageid"), "", "languageid = ".sqlVal($lang));
    if(Core::getDB()->num_rows($result) > 0)
    {
      $this->langId = $lang;
      return $this;
    }
    throw new GenericException("The given language id does not exist. Please create language first before import data.");
  }
  ...
  */
  ...
  {
    $result = Core::getQuery()->select("languages", array("languageid"), "", "langcode = ".sqlVal($lang));
    ...
    if(!$row)
    {
      ...
      {
        throw new GenericException("The given language code does not exist. Please create language first before import data.");
      }
    }
  ...
  */
  ...
  {
    $result = Core::getQuery()->select("phrasesgroups", "phrasegroupid", "", "phrasegroupid = ".sqlVal($group));
    if(Core::getDB()->num_rows($result) > 0)
    {
      $this->groupId = $group;
      return $this;
    }
    throw new GenericException("The given phrase group does not exist. Please create group first before import data.");
  }
  ...
  */
  ...
  {
    $result = Core::getQuery()->select("phrasesgroups", "phrasegroupid", "", "title = ".sqlVal($group));
    $row = Core::getDB()->fetch($result);
    Core::getDB()->free_result($result);
    if(!$row)
    {
  ...
  */
  public function setDefaultLanguage($langId)
  {
    ...
    {
      $result = Core::getQuery()->select("languages", "languageid", "", "langcode = ".sqlVal($langId), "", "1");
