...
* @version $Id: Upload.util.class.php 23 2010-04-03 19:08:34Z craft $
*/
...
class Upload
{
  ...
  */
  public function __construct($path, $file)
  {
    ...
    // Hook::event("START_FILE_UPLOAD", array(&$this));
    return;
  }
  ...
  */
  public function check($throwExceptions = true)
  {
    if(!$this->file || !isset($this->file) || !$this->filename || $this->filename == "" || strlen($this->filename) == 0)
    {
      ...
      if($throwExceptions) { throw new IssueException("NO_FILE_CHOSEN"); }
      return $this;
    }
    ...
    {
      $this->canUpload = false;
      if($throwExceptions) { throw new IssueException("PATH_DOES_NOT_EXIST"); }
      return $this;
    }
    if($this->validateFileName())
    {
      $this->canUpload = false;
      if($throwExceptions) { throw new IssueException("FILE_ALREADY_EXISTS"); }
      return $this;
    }
    ...
    if($allowedExtensions = $this->getAllowedFileExtensions())
    {
      ...
      {
        $this->canUpload = false;
        if($throwExceptions) { throw new IssueException("INVALID_FILE_EXTENSION"); }
        return $this;
      }
    }
    ...
    {
      $this->canUpload = false;
      if($throwExceptions) { throw new IssueException("FILE_SIZE_TOO_LARGE"); }
      return $this;
    }
    // Hook::event("CHECK_FILE_BEFORE_UPLOAD", array(&$this));
    return $this;
  }
  ...
  */
  public function addStringToFilename($string)
  {
    if(Str::length($string) > 0)
    {
      ...
      // Hook::event("ADD_STRING_TO_FILENAME", array(&$this));
    }
    return $this;
  }
  ...
  */
  public function doUpload()
  {
    if($this->canUpload && ini_get("file_uploads"))
    {
      ...
      {
        throw new GenericException("Error with uploading file '".$this->filename."'.");
      }
      // Hook::event("PERFORM_FILE_UPLOAD", array(&$this));
    }
    return $this;
  }
  ...
  */
  protected function getPHPMaxUploadSize()
  {
    ...
    $qty = intval(Str::substring($ini_get, 0, $value_length - 1));
  ...
  */
  public function setAllowedFileExtensions($extensions)
  {
    ...
    {
      throw new GenericException("The supplied extensions must be an array, string or map.");
    }
    return $this;
  }
