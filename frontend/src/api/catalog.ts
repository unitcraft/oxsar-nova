...
export const SHIPS: CombatEntry[] = [
  { id: 29, key: 'small_transporter', name: 'Малый транспорт',    attack: 5,      shield: 10,    shell: 4000,    cargo: 5000,    speed: 5000,      fuel: 10,   cost: { metal: 2000,    silicon: 2000,   hydrogen: 0       }, requires: [{ kind: 'building', key: 'shipyard', level: 2 }, { kind: 'research', key: 'combustion_engine', level: 2 }], description: 'Дешёвый транспорт для перевозки ресурсов' },
  { id: 30, key: 'large_transporter', name: 'Большой транспорт',  attack: 5,      shield: 25,    shell: 12000,   cargo: 25000,   speed: 7500,      fuel: 50,   cost: { metal: 6000,    silicon: 6000,   hydrogen: 0       }, requires: [{ kind: 'building', key: 'shipyard', level: 4 }, { kind: 'research', key: 'combustion_engine', level: 6 }], description: 'Основной грузовоз — вмещает 25 000 ед.' },
  { id: 31, key: 'light_fighter',     name: 'Легкий истребитель', attack: 50,     shield: 10,    shell: 4000,    cargo: 50,      speed: 12500,     fuel: 20,   cost: { metal: 3000,    silicon: 1000,   hydrogen: 0       }, requires: [{ kind: 'building', key: 'shipyard', level: 1 }, { kind: 'research', key: 'combustion_engine', level: 1 }], description: 'Дешёвый и быстрый — основа атакующего флота' },
  ...
  { id: 34, key: 'battle_ship',       name: 'Линкор',             attack: 1000,   shield: 200,   shell: 60000,   cargo: 1500,    speed: 10000,     fuel: 500,  cost: { metal: 45000,   silicon: 15000,  hydrogen: 0       }, requires: [{ kind: 'building', key: 'shipyard', level: 7 }, { kind: 'research', key: 'hyperspace_engine', level: 4 }], description: 'Мощный боевой корабль с гиперпространственным двигателем' },
  { id: 36, key: 'colony_ship',       name: 'Колонизатор',        attack: 50,     shield: 100,   shell: 30000,   cargo: 7500,    speed: 2500,      fuel: 1000, cost: { metal: 10000,   silicon: 20000,  hydrogen: 10000   }, requires: [{ kind: 'building', key: 'shipyard', level: 4 }, { kind: 'research', key: 'impulse_engine', level: 3 }], description: 'Позволяет колонизировать незанятые планеты' },
  ...
  { id: 39, key: 'solar_satellite',   name: 'Солнечный спутник',  attack: 1,      shield: 1,     shell: 2000,                    speed: 5000,      fuel: 0,    cost: { metal: 0,       silicon: 2000,   hydrogen: 500     }, requires: [{ kind: 'building', key: 'shipyard', level: 1 }], description: 'Добавляет энергию без строительства электростанций' },
  ...
  { id: 42, key: 'death_star',        name: 'Звезда смерти',      attack: 200000, shield: 50000, shell: 9000000, cargo: 1000000, speed: 100,       fuel: 1,    cost: { metal: 5000000, silicon: 4000000,hydrogen: 1000000 }, requires: [{ kind: 'building', key: 'shipyard', level: 12 }, { kind: 'research', key: 'hyperspace_tech', level: 6 }, { kind: 'research', key: 'hyperspace_engine', level: 7 }], description: 'Сильнейший корабль — способен уничтожить луну' },
];

export const DEFENSE: CombatEntry[] = [
  { id: 43, key: 'rocket_launcher', name: 'Ракетная установка',   attack: 80,   shield: 20,    shell: 2000,    cost: { metal: 2000,  silicon: 0,    hydrogen: 0    }, requires: [{ kind: 'building', key: 'shipyard', level: 1 }], description: 'Базовая и дешёвая оборонительная установка' },
  { id: 44, key: 'light_laser',     name: 'Легкий лазер',         attack: 100,  shield: 25,    shell: 2000,    cost: { metal: 1500,  silicon: 500,  hydrogen: 0    }, requires: [{ kind: 'building', key: 'shipyard', level: 2 }, { kind: 'research', key: 'laser_tech', level: 3 }], description: 'Лазерная пушка начального уровня' },
  { id: 45, key: 'strong_laser',    name: 'Тяжелый лазер',        attack: 250,  shield: 100,   shell: 8000,    cost: { metal: 6000,  silicon: 2000, hydrogen: 0    }, requires: [{ kind: 'building', key: 'shipyard', level: 4 }, { kind: 'research', key: 'laser_tech', level: 6 }, { kind: 'research', key: 'energy_tech', level: 3 }], description: 'Усиленная лазерная пушка с большей мощностью' },
  { id: 47, key: 'gauss_gun',       name: 'Пушка Гаусса',         attack: 1100, shield: 200,   shell: 35000,   cost: { metal: 20000, silicon: 15000,hydrogen: 2000 }, requires: [{ kind: 'building', key: 'shipyard', level: 6 }, { kind: 'research', key: 'gun_tech', level: 3 }, { kind: 'research', key: 'shield_tech', level: 1 }, { kind: 'research', key: 'energy_tech', level: 6 }], description: 'Мощная пушка — эффективна против тяжёлых кораблей' },
  { id: 48, key: 'plasma_gun',      name: 'Плазменное орудие',    attack: 3000, shield: 300,   shell: 100000,  cost: { metal: 50000, silicon: 50000,hydrogen: 30000}, requires: [{ kind: 'building', key: 'shipyard', level: 8 }, { kind: 'research', key: 'plasma_tech', level: 7 }], description: 'Наиболее разрушительное орудие обороны' },
  { id: 49, key: 'small_shield',    name: 'Малый щитовой купол',  attack: 1,    shield: 2000,  shell: 20000,   cost: { metal: 10000, silicon: 10000,hydrogen: 0    }, requires: [{ kind: 'building', key: 'shipyard', level: 1 }, { kind: 'research', key: 'shield_tech', level: 2 }], description: 'Защищает всю оборону планеты от одного залпа' },
  { id: 50, key: 'large_shield',    name: 'Большой щитовой купол',attack: 1,    shield: 10000, shell: 100000,  cost: { metal: 50000, silicon: 50000,hydrogen: 0    }, requires: [{ kind: 'building', key: 'shipyard', level: 6 }, { kind: 'research', key: 'shield_tech', level: 6 }], description: 'Усиленный купол — щит в 5× больше малого' },
];
