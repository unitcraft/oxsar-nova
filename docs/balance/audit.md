...
- **Статус**: **patched** 2026-05-01 (план 72.1.3, возврат к legacy 50%/1%)
...
**Фикс** (применён 2026-05-01): в [attack.go::calcDebris](../../projects/game-nova/backend/internal/fleet/attack.go)
введены константы `debrisShipsFactor=50` и `debrisDefenseFactor=1`
(в процентах). Логика теперь различает ships/defense через
`defenseIDs map`: ships → 50% (Java fleet bulk), defense → 1%
(Java defense bulk). Hydrogen в обломки не идёт (legacy идентично).
Cost подбирается сначала в `cat.Ships`, потом в `cat.Defense.Defense`
(map[name]DefenseSpec).

**Тесты** (3 case в `attack_test.go`):
- `TestCalcDebris_Basic` — ships 50% (обновлён, было 30%).
- `TestCalcDebris_DefenseGives1Percent` — новый: defense 1%.
- `TestCalcDebris_MixedShipsAndDefense` — новый: смешанная сторона
  с ships+defense, проверка обоих factor одновременно.
...
- **Статус**: **patched** 2026-05-01 (план 72.1.3, defence-in-depth)
...
**Фикс** (применён 2026-05-01): добавлено поле
`Input.IsRocketAttack bool`. В `Calculate` ([engine.go:140](../../projects/game-nova/backend/internal/battle/engine.go#L140))
guard `&& !in.IsRocketAttack` блокирует расчёт опыта. Тест
`TestExperience_RocketAttack_NoExp` проверяет что обычный бой даёт
опыт > 0, а IPM-режим — exp=0 для обеих сторон. Defence-in-depth:
сейчас `internal/rocket/` не вызывает `Calculate`, но если в
будущем перейдёт — работа будет 1:1 с Java.
...
- **Статус**: **not-a-bug** 2026-05-01 (план 72.1.3, проверено по Java)
...
**Закрытие 2026-05-01 (план 72.1.3, not-a-bug)**: проверено в
[Party.java:479](../../../oxsar2-java/assault/src/assault/Party.java#L479):
...
Java делает **ровно тот же `clamp(.., 1, ..)`** что и Go
(`if rawShots <= 0 { rawShots = 1 }` в [engine.go:758-760](../../projects/game-nova/backend/internal/battle/engine.go#L758)).
Семантика идентична: каждая активная цель получает минимум 1 выстрел.
...
**Закрытие 2026-05-01 (план 72.1.3, not-a-bug)**: ошибка моей
интерпретации при аудите. Проверено в
[Units.java:315-317](../../../oxsar2-java/assault/src/assault/Units.java#L315):
...
Java тоже применяет rapidfire **per-target** — `getRapidFire`
вызывается с конкретной целью `underFireUnits.getUnitid()`. То же
поведение что Go ([engine.go:761-762](../../projects/game-nova/backend/internal/battle/engine.go#L761)):
`rapidfireMult(rf, shooter, tgt) * rawShots`.
...
**Закрытие 2026-05-01 (план 72.1.3, not-a-bug)**: проверено по Java:
- [Units.java:243](../../../oxsar2-java/assault/src/assault/Units.java#L243) — `weight = pow(2, front)`.
- [Units.java:89-91](../../../oxsar2-java/assault/src/assault/Units.java#L89) — `getStartTurnWeight() = weight × quantity`.
- [Party.java:478](../../../oxsar2-java/assault/src/assault/Party.java#L478) —
  `attackQuantity = round(quantity * targetWeight / totalWeight)` —
  **точно та же формула**, что у Go в [engine.go:756-757](../../projects/game-nova/backend/internal/battle/engine.go#L756).
- Семантика идентична: высокий `front` → больше weight → больше
  выстрелов получает юнит (front-line absorber). Это **front-line
  модель abosrber**, как и реализовано.
