...
**Принцип:** один backend на обе вселенные, UI — отдельно для каждой
по триггеру.

- **Backend** (Ф.1) — пишется сразу для **обеих** вселенных. State,
  API, метрики, миграция — universe-aware через R10. Стоимость
  «universe-aware с самого начала» = +0 строк (R10 это базовое
  правило).
- **nova-фронт UI** (Ф.2-Ф.5) — полный onboarding по этому плану.
  Запуск после плана 74.
- **origin-фронт UI** — **отдельный план 82** (или Ф.7 этого плана),
  запускается **только по триггеру**:
  - drop-off на первом часе игры в origin-вселенной > 30% за первые
    100 регистраций, ИЛИ
  - >5 жалоб от игроков «не понятно что делать в origin»,
  - либо явное продуктовое решение «origin = массовая аудитория, не
    только ветераны».
  Если триггеров нет — origin-фронт остаётся без onboarding (гипотеза:
  он целит в опытных legacy-ветеранов, tutorial мешает).

**Контент шагов tutorial** для origin-фронта (если будет делаться)
потребует адаптации к legacy-терминологии (hf-фабрика → fusion-reactor
по legacy-номенклатуре, и т.д.). Это **не дополнительные шаги**, а
переписывание i18n-текстов под legacy-словарь.
...
- **Не делаем UI для origin-фронта в этом плане** — backend пишется
  под обе вселенные, но origin-UI выделен в отдельный план 82,
  запускается по триггеру drop-off (см. «Применимость по вселенным»).
