...
### Этап 4 — UI — ✅ MVP ЗАКРЫТО 2026-04-25

Реализовано:
- Backend: `GET /api/alien/holdings/me` → список активных HOLDING на
  планетах игрока. Возвращает event_id, координаты, тир, alien-флот,
  start/ends/max-ends, paid-инфу.
- Frontend: `AlienHoldingPanel` подключён в OverviewScreen. Показывает
  карточки захваченных планет с таймером до ухода, составом флота и
  формой платежа (ввод суммы + кнопка «Заплатить»).
- Полностью использует уже существующий `POST /api/alien/holding/{id}/pay`
  (этап 1 + 2).

Smoke в Docker: 401 без auth, корректный routing.

Отложено в v1.x (не блокер):
- [ ] Индикатор «занято пришельцами» в GalaxyScreen для других игроков.
- [ ] Кнопки действий прямо в почте (open modal vs deep-link).
