...
- **Ракеты из галактики**: кнопка 🚀 в MissionButtons → RocketPanel (выбор планеты-источника,
  кол-во ракет, запрос наличия) → POST /api/planets/{id}/rockets/launch
- **Экспедиции из галактики**: кнопка 🌌 для пустых позиций и собственных планет → onFleetMission(15)
- **Водород/расстояние**: tooltip на кнопках миссий показывает расстояние и время полёта
  (минимальная скорость флота × 100%)
- **Star Surveillance**: кнопка 👁 сохраняет систему в localStorage;
  список наблюдаемых систем отображается над таблицей как кнопки-закладки
