...
- Найти UGC-точки через grep по `getPOST` / `INSERT INTO`.
- Для каждой точки применить `isForbidden()` (блокировка) или `mask()`
  (маскирование) согласно таблице.
- При необходимости расширить API `Moderation::mask()` по образцу
  `MaskForbidden` из `projects/game-nova/backend/internal/moderation/blacklist.go`.
- Аудит-таблица **не нужна** в Ф.4.
- Smoke-тест из 8 шагов вручную через dev-вселенную.
- 1 общий коммит (или 2 — расширение API + применение).
