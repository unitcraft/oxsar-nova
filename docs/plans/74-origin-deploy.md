...
- Зарегистрировать поддомен по решению ADR-0010
  (`origin.oxsar-nova.ru` / `legacy01.oxsar-nova.ru` / другое).
...
- Регистрация в `universes` registry (план 36): запись с
  `code = '<chosen>'` (по ADR-0010), `display_name = '<russian>'`,
  `balance_profile = 'legacy'`.
...
- `universes.code` — snake_case (`origin_classic`, `legacy01` или
  что выберет ADR-0010), не camelCase.
