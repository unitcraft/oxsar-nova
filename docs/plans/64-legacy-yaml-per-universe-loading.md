...
обслуживать **две вселенные** с разным балансом:

- `uni01`, `uni02` (modern) — текущие YAML / Go-формулы nova,
  никаких изменений в значениях, ребалансы планов 17/18/20/21
  остаются как есть.
- `legacy01` (legacy) — балансовые числа из game-origin
  (`na_construction` + `consts.php`).

Каждая вселенная знает свой профиль (`universe.balance_profile =
'modern' | 'legacy'`), и при любых балансовых вычислениях
backend читает значения из соответствующего источника.

---

## Контекст

### Почему «один backend, два профиля»

Стратегия ремастера зафиксирована планом 62: один движок
(game-nova), две вселенные. Современная (uni01) — текущий nova-баланс.
Легаси (legacy01) — pixel-perfect баланс origin для игроков-«классиков».
...
### Почему legacy.yaml + Go-формулы (B1+B3 гибрид)
...
- **B1 — статика в YAML.** Числа `basic_*`, плюс **предвычисленные
  ...
  скриптом — лежит в `configs/balance/legacy.yaml` как факт.
...
### 1. БД — поле `universes.balance_profile`

Миграция: новая колонка в существующей таблице вселенных.

```sql
-- migrations/00NN_add_balance_profile.sql
ALTER TABLE universes
  ADD COLUMN balance_profile TEXT NOT NULL DEFAULT 'modern'
    CHECK (balance_profile IN ('modern', 'legacy'));

CREATE INDEX idx_universes_balance_profile
  ON universes(balance_profile);

COMMENT ON COLUMN universes.balance_profile IS
  'Балансовый профиль: modern (uni01/uni02 — current nova balance) '
  'или legacy (legacy01 — origin/oxsar2 balance, см. configs/balance/legacy.yaml).';
```

По R1: имя — `balance_profile` (snake_case, осмысленный префикс
`balance_*`, не `profile_id` без контекста), тип — TEXT с CHECK
(не int с magic numbers, не enum-таблица для двух значений).

### 2. Конфиг `configs/balance/legacy.yaml`
...
```yaml
# configs/balance/legacy.yaml
# Балансовый профиль legacy (origin/oxsar2).
...
6. Записывает результат в `configs/balance/legacy.yaml`.
...
// Load — загружает балансовый bundle для конкретного профиля.
// modern читает существующие configs/{buildings,units,rapidfire}.yml.
// legacy читает configs/balance/legacy.yaml.
func Load(profile Profile) (*Bundle, error) { ... }

// LoadFor — удобный wrapper: загружает bundle по universe-id,
// читая profile из БД.
func LoadFor(ctx context.Context, db DB, universeID string) (*Bundle, error)
```

**Кешируется in-memory** (один раз при старте процесса; на смену
профиля во время работы пересобирается через signal/admin-API).
...
- **Не реализуем полноценный DSL-evaluator на Go.** Только
  ...
  Go использует числа.
- **Не вводим UI** для смены balance_profile (через админку или
  игровой UI). На старте — миграция SQL-ом ставит профиль для
  каждой вселенной, дальше не меняется.
- **Не мигрируем данные** между профилями (игроки uni01 не
  переходят в legacy01 и обратно — у них разные вселенные).
...
### Ф.1. Скаффолд + миграция БД

- Создать миграцию `migrations/00NN_add_balance_profile.sql`.
- Создать скелет `internal/balance/loader.go` с типами + базовый
  `Load(ProfileModern)` — должен вернуть текущий nova-bundle
  через существующие YAML-loader'ы (рефакторинг существующего кода).
- Прогнать миграцию в dev — проверить, что existing universes
  получают `balance_profile='modern'`.
- Тест: `Load(ProfileModern)` возвращает ту же конфигурацию, что
  раньше читалась прямым доступом к YAML.
...
- Генерация `configs/balance/legacy.yaml`.
- Запуск: должен выдать YAML на ~3000-5000 строк.
...
### Ф.3. Loader для legacy профиля

- Расширить `internal/balance/loader.go` — `Load(ProfileLegacy)`
  читает `configs/balance/legacy.yaml`.
- `LoadFor(ctx, db, universeID)` — выбор по `universes.balance_profile`.
- In-memory кеш с invalidation через канал (на будущее).
- Тест: создать тестовую вселенную с `balance_profile='legacy'`,
  проверить, что `bundle.Buildings["metal_mine"].Basic.Metal == 60`.
...
- Создать первый сценарий legacy: тест-вселенная с
  `balance_profile='legacy'`, проверить, что у неё свои числа.

### Ф.6. E2E + Smoke

- Поднять dev-стенд game-nova с двумя вселенными:
  `uni01` (modern), `legacy_test` (legacy).
- Создать пользователя в каждой, поставить здание в очередь.
- Verify: стоимость постройки в `legacy_test` соответствует
  `legacy.yaml` (Metal Mine lvl 1 → 60/15/0/0), в `uni01` —
  текущий nova.
- Verify: тик производства даёт legacy-числа в legacy_test.
...
- YAML: ~3000-5000 строк автогенерации (`legacy.yaml`).
...
2. **`feat(balance): импорт legacy → configs/balance/legacy.yaml (план 64 Ф.2)`**
...
- Колонка БД: `balance_profile` (snake_case, с осмысленным
  префиксом `balance_*`).
- Тип колонки: TEXT с CHECK, не int с magic numbers.
...
- JSON-поля (если будут в admin-API): `balance_profile` в snake_case.
...
| Импортёр пропустил формулу (например, новый оператор в DSL) | После генерации `legacy.yaml` — diff-тест: сравнить количество записей с `SELECT COUNT FROM na_construction`; alert при несоответствии |
...
- `legacy.yaml` — фиксированная база легаси-чисел в репо.
- Динамические производственные формулы реализованы для legacy.
- Разблокированы планы:
  - **65** (расширение event-loop) — теперь есть откуда брать
    legacy-стоимости постройки.
  - **66** (AlienAI до полного паритета) — алиен-юниты
    `UNIT_A_*` уже в `legacy.yaml`.
  - **69** (расширение domain-полей) — может опираться на
    legacy.yaml для legacy-значений.
