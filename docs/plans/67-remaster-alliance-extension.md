...
**Дата**: 2026-04-28
**Статус**: ✅ ЗАКРЫТ 2026-04-28. Ф.0 дельта-аудит ✅; Ф.1 миграции ✅
(коммит 99895ae230); Ф.2 backend handlers + RBAC + дипстатусы +
audit-log ✅ (коммит 2fd010cd87); Ф.3 transfer-leadership с
email-кодом ✅ (коммит 556baa8483); Ф.4 полнотекстовый поиск +
GIN-индекс ✅ (коммит d7988572c4); Ф.5 ч.1 frontend (descriptions +
ranks + diplomacy) ✅ (коммит 669af55dae); Ф.5 ч.2 frontend
(audit-log + search + transfer-leadership) ✅ + Ф.6 финализация ✅
(этот коммит).
...
## Ф.5 ч.2 Реализация (2026-04-28) — frontend

**Новые файлы:**
- `frontends/nova/src/features/alliance/AuditLogPanel.tsx` — UI журнала
  событий: фильтры по action (18 known) и actor (dropdown членов),
  пагинация offset/limit (50/page), relative-time формат через
  `formatRelativeTime` («5m ago» / «2h ago» / «3d ago»).
- `frontends/nova/src/features/alliance/AllianceSearchPanel.tsx` —
  фильтры списка альянсов: q (полнотекст), openness (all/open/closed),
  min/max members, кнопка reset. Контролируемый компонент, debounce
  выполняется в родителе (300ms).
- `frontends/nova/src/features/alliance/TransferLeadershipDialog.tsx` —
  модалка передачи лидерства: 2-step flow (radio-выбор кандидата →
  ввод 8-символьного кода из system-message). Idempotency-Key
  переиспользуется между POST /code и POST /transfer-leadership для
  одной транзакции R9. Локализация ошибок: invalid_code / expired_code
  / not_a_member / not_owner / billing_unavailable / rate_limited /
  attempts_exhausted.
- `frontends/nova/src/features/alliance/audit.ts` — pure-helpers:
  18-элементный `AUDIT_ACTIONS` (зеркало backend `audit.go`),
  `auditActionLabelKey` / `auditTargetKindLabelKey` с fallback на
  unknown, `formatRelativeTime` (clamp на 0 для будущих дат, минуты
  / часы / дни / месяцы).
- `frontends/nova/src/features/alliance/search-filters.ts` — типы +
  `buildSearchQuery` (URLSearchParams, валидирует non-negative integer
  для min/max, частично заполненные диапазоны передаются как есть).
- `audit.test.ts` (15 кейсов) и `search-filters.test.ts` (13 кейсов)
  — pure-function unit-тесты в стиле permissions.test.ts/relations.test.ts
  Ф.5 ч.1.

**Изменённые файлы:**
- `frontends/nova/src/features/alliance/AllianceScreen.tsx` —
  интеграция: панель фильтров над листингом + debouncedQuery
  через useEffect/300ms, кнопка «👑 Передать лидерство» в header
  card (видна только owner'у, скрыта если он один в альянсе),
  AuditLogPanel секцией после DiplomacyPanel, открытие
  TransferLeadershipDialog по кнопке.
- `configs/i18n/ru.yml` + `configs/i18n/en.yml` — добавлены ключи
  под секцией `alliance:`: `audit.*` (~33 ключа: title, фильтры,
  колонки, 18 actions, 4 target_kind, time-ago на 5 интервалов),
  `search.*` (9 ключей), `transferLeadership.*` (17 ключей с
  локализацией всех known error codes). Переиспользованы:
  `confirmBtn`, `cancelBtn`, `acceptBtn`, `rejectBtn`, `colMember`,
  `colActions`, `diplomacy.filterAll`, `disbandBtn`, `descriptions.title`,
  `ranks.title`, `sectionRelations`, `transferLeadership.codeTitle`/
  `codeBody` (system-message bodies из Ф.3).

**Дизайн-решения Ф.5 ч.2:**

1. **Тесты как pure helpers, не RTL.** В проекте нет `@testing-library/react`
   и React Testing Library setup'а; стиль Ф.5 ч.1 (permissions.test.ts +
   relations.test.ts) — именно pure-function-тесты. Подняли по той же
   модели: 28 новых юнит-тестов для `audit.ts` и `search-filters.ts`,
   все 42 теста секции alliance проходят. Добавление RTL — отдельная
   инфра-задача, не входит в Ф.5 ч.2.

2. **Ключ `Idempotency-Key` для transfer-leadership.** Code-request
   и confirm — две стадии одной операции; используем разные ключи,
   но одинаковый ключ для retry внутри стадии (R9). Code-key хранится
   в state до закрытия модалки; confirm-key обнуляется при ошибке
   (например, `expired_code` → пользователь запрашивает новый код,
   будет новая транзакция).

3. **Дебаунс в search.** 300ms через `useEffect + setTimeout` в
   родителе (AllianceScreen), не в SearchPanel. Это держит queryKey
   TanStack Query стабильным на каждом keystroke — кеш не разрастается
   мусором. Альтернатива (`useDeferredValue`) тянула бы лишний рендер
   на каждый ввод.

4. **Audit-log в `MyAlliancePanel`, не в `AllianceDetail`.** Бэкенд
   ограничивает доступ только членами альянса; показывать его
   outsider'у в детали-карточке нет смысла. Логично выводить в
   секции «Мой альянс».

5. **Относительное время в audit, не absolute.** «5m ago» по
   `formatRelativeTime` — для скана списка событий; abs-время
   доступно через `title` колонки `colWhen`. Локализация — через
   `audit.time.{justNow|mAgo|hAgo|dAgo|moAgo}` ключи.

6. **R12 переиспользование ~17%.** Существенно ниже целевых 95% Ф.5 ч.1
   — это ожидаемо: audit-log и transfer-leadership — полностью новые
   UX-домены без аналогов в существующих экранах. 18 action-меток
   физически не могли быть переиспользованы. R15-нарушением не считаем.

## Ф.6 Финализация (2026-04-28)

- divergence-log: D-014, D-040, D-041 → ✅ ЗАКРЫТО.
- nova-ui-backlog: U-004, U-005, U-012, U-013, U-015 → ✅ ЗАКРЫТО.
- Шапка плана 67: весь план ✅ ЗАКРЫТ.
- docs/project-creation.txt: запись итерации Ф.5 ч.2 + Ф.6.
