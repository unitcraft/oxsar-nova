...
### Сравнение шлюзов (Россия, 2025) — без ИП/юрлица

| Шлюз | Самозанятый/физлицо | Комиссия | Выплаты | Особенности |
|------|---------------------|----------|---------|-------------|
| **ЮKassa** | ✅ Самозанятый (НПД) | 2.8–3.5% | В день платежа | API-чек ФНС в «Мой налог», карты + СБП + кошельки, зрелые SDK |
...
**Выбор: ЮKassa** — основной (план 42, реализация в `billing-service`).
Робокасса и Enot.io — резервные (через тот же `Gateway` интерфейс,
порт из game-nova при появлении тестового аккаунта).

Причины выбора ЮKassa:
- автоматическая регистрация чека ФНС в «Мой налог» через API (`receipt`
  в `POST /v3/payments`) — без ручной выписки;
- широкая поддержка методов оплаты;
- зрелая документация и Go-SDK.

См. [42-yookassa.md](42-yookassa.md) и
[../ops/payment-integration.md](../ops/payment-integration.md).
...
- [ ] Зарегистрироваться на robokassa.com (тип учётной записи —
      физлицо/самозанятый, в зависимости от налогового режима автора)
