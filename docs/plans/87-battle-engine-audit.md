...
**Дата**: 2026-05-01
**Статус**: ✅ Закрыт (2026-05-01) — **audit-этап**.
**Переоформление 2026-05-01**: следующие итерации (закрытие 14 open
находок) перенесены в [план 72.1.3](72.1.3-battle-engine-audit-fixes.md)
как подплан 72.1 (с ограничениями родителя §11 «без упрощений», §3
i18n). Этот план остаётся как историческая запись audit-этапа: цель,
методология, факт находок. Список 15 BA-записей — единый
[docs/balance/audit.md](../balance/audit.md), туда же пишутся апдейты
patched/not-a-fix.

> **Итог**: глубокий audit прочёл `engine.go` (1040 строк) + `types.go` +
> `simstats.go` + 5 call sites. Сверка с reference-Java (`d:\Sources\oxsar2-java`).
> Найдено **15 проблем**, все зафиксированы как BA-007..BA-021 в
> [docs/balance/audit.md](../balance/audit.md). Распределение severity:
> **3 P0/P1** (опыт занижен 2× в обычных боях, debris-формула расходится с
> legacy, отсутствие destroy/moon-механик), **6 P1/P2**, **6 P3/info**.
> Самый болезненный регресс — **BA-007**: `IsMoon` передаётся в
> `computeExperience` как `hasPlanet`, что переворачивает семантику и
> делает `bpc *= 0.5` для **обычных атак планет**. Тест на это есть, но
> вызывает `computeExperience` напрямую с заранее заданным `hasPlanet`,
> минуя `Calculate` — поэтому баг не ловится.
>
> Не правил ни строчки кода. Все 15 находок — **open**, фиксы оставлены
> отдельным планам.
