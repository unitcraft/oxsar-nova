...
**Домен**: `projects/game-origin-php/`  
...
6. Привести структуру файлов к чистому виду внутри `projects/game-origin-php/`.
...
Скопировать в `projects/game-origin-php/` всё из `d:\Sources\oxsar2\www\`, включая:
...
- Все `require`/`include` пути обновляются под новую структуру `projects/game-origin-php/`
...
- Новый воркер: `projects/game-origin-php/worker/` — отдельный бинарь, запускается рядом
  с PHP-приложением.
...
## Структура `projects/game-origin-php/`
...
| # | Задача | Риск |
|---|---|---|
| 37.1 | Слепое копирование всех файлов в `projects/game-origin-php/` | низкий |
...
- Новый файл `projects/game-origin-php/src/game/OnboardingService.class.php`
...
#### 2. Воркер `projects/game-origin-php/worker/event-monitor.php`
...
```bash
mkdir -p projects/game-origin-php/migrations/fixtures
docker exec oxsar2-mysql-1 mysqldump -uroot -proot oxsar_db \
  --no-create-info --skip-extended-insert --skip-comments --hex-blob \
  --where="userid=1" \
  ...
  2>/dev/null > projects/game-origin-php/migrations/fixtures/test-user-snapshot.sql
...
Скрипт `projects/game-origin-php/tools/apply-test-user-fixture.sh`:
```bash
#!/bin/bash
...
Скрипт `projects/game-origin-php/tools/compare-with-legacy.sh`:
```bash
#!/bin/bash
...
([NS.class.php:140](../../projects/game-origin-php/src/game/NS.class.php#L140)),
...
**(1) nginx-конфиг** (`projects/game-origin-php/docker/nginx.conf`).
Раньше:
...
**(2) PHP-парсинг PATH_INFO** (`projects/game-origin-php/src/core/Request.class.php`).
...
`projects/game-origin-php/src/core/Debuger.php`:
