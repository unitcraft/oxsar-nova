...
**Обновление 2026-04-30**: побочный тезис «временная legacy-реализация, удалится после готовности нового фронта» (см. §Цель и §Ф.5 ниже) **отозван**. `projects/game-legacy-php/` (текущее имя после плана 78) остаётся в проекте как параллельная PHP-реализация и pixel-perfect референс. Сам план 75 — про переименование путей — выполнен и не пересматривается; меняется только намерение «удалить позже».
