...
### 17.0 Как читать exchange ratio в матрицах

**Формула**: `exchange = def_loss / atk_loss` (потери защитника / потери
атакующего, в metal-eq).

**Условия теста**: обе стороны строятся из **одного юнита** под завязку
бюджета (10M metal-eq по умолчанию).
- Cost=5k Shadow → 10M / 5k = **2000 Shadow**
- Cost=15k Shadow (после 27-V) → 10M / 15k = **666 Shadow**
- LF cost=4k → 10M / 4k = **2500 LF**

После боя cравниваем суммарные потери ресурсов с обеих сторон.

**Шкала**:

| ratio | смысл |
|---:|---|
| `0.10` | На 10k моих потерь враг теряет 1k → **я в убытке** |
| `1.00` | Равный обмен |
| `2.00` | На 1k моих потерь враг теряет 2k → **я в плюсе** |
| `10.00` | На 1k моих потерь враг теряет 10k → **сильный counter** |
| `90.00` | На 1k моих потерь враг теряет 90k → **доминанта (плохо для баланса)** |

**Целевой коридор**: большинство пар должно быть в **0.3–3.0**
(никто сильно не доминирует). Допустимы выбросы:
- Anti-counter через RF (Cruiser RF×6 vs LF) → 3-10 — норма для роли
  «counter».
- Anti-mass специалист (Bomber RF×20 vs RL) → 15-20 — норма.
- DS endgame vs обычный флот → ATK clean (DS не теряет) — by design.

**Что считается «слишком много»**:
- `>10` для не-counter pair — флаг доминанты.
- `>50` — катастрофа (юнит обнуляет целый tier).

**Спец-обозначения в ячейках**:
- `ATK` = атакующий не теряет ничего (clean victory).
- `def` = атакующий проигрывает (defender clean).
- `—` = оба сохраняют флот (никто не пробивает).

**Пример: Shadow vs LF (90.91 → 7.57 после 27-V)**:
- До: 2000 Shadow убивают почти всех LF (2500 × 4k = 10M loss защитника),
  сами теряют ~110k. exchange = 10M / 110k = 90.91. Любой флот LF
  теряется бесплатно для Shadow → mета сломана.
- После: 666 Shadow (в 3× меньше) — меньше выстрелов в раунд, LF успевает
  отстреливаться. Atk loss ~1.3M, def loss ~10M → exchange 7.57. Shadow
  всё ещё доминирует, но теряет 13% флота — это уже **счётный размен**.
...
## 20. 27-W' Lancer без cap (sweep-анализ, 2026-04-26)

После применения cap=50 возник вопрос: **можно ли решить Lancer-spam
проблему чистым ребалансом без game-механики cap**?

Систематически проверил каждую характеристику Lancer'а sweep-тестами
(шаг = одно изменение, остальное в baseline 27-W: attack=5000,
shell=10000, cost 110k, front=6, RF Lancer→DS×3).

### 20.1 Sweep-таблица (lancer-vs-mixed: 500 Lancer vs lite)

| Изменение | atk_wins | atk_loss | def_loss | Lancer-spam? |
|---|---:|---:|---:|---|
| **Baseline 27-W** (atk=5000) | 100% | 64% | 100% | ❌ работает |
| attack=3000 | 0% | 100% | 59% | ✅ убит, но Lancer-trap 1v1 |
| attack=3500 | 0% | 92% | 72% | ✅ убит |
| attack=4000 | 0% | 80% | 86% | ✅ убит |
| attack=4500 | 100% | 70% | 100% | ❌ работает |
| shell=5000 (хрупкий) | 0% | 100% | 42% | ✅ убит |
| shell=7500 | 0% | 100% | 72% | ✅ убит |
| shell=15000 (танк) | 100% | 39% | 100% | ❌ хуже! |
| cost=150k (+36%) | 100% | 64% | 100% | ❌ не помогает |
| front=4 (stealth) | 100% | 64% | 100% | ❌ не помогает |
| front=10 (как все) | 100% | 64% | 100% | ❌ не помогает |
| LF→Lancer RF×40 | 0% | 100% | 32% | ✅ убит |
| **Cruiser→Lancer RF×45** ⭐ | 0% | 96% | 81% | ✅ **убит** |
| Cruiser→Lancer RF×50 | 0% | 100% | 65% | ✅ убит |

### 20.2 Что РЕАЛЬНО влияет

| Параметр | Влияние | Почему |
|---|---|---|
| **attack** | ⭐⭐⭐ сильное | Прямая damage-output. ≤4000 → не дострелит lite за 6 раундов |
| **shell** | ⭐⭐⭐ сильное | ≤7500 → Lancer гибнет от Cruiser×35 быстрее |
| **RF Cruiser→Lancer** | ⭐⭐⭐ сильное | Точечный counter именно vs Lancer-mass |
| **cost** | ⭐ слабое | Не меняет боевое поведение, только чистую экономику |
| **front** | ⭐ слабое | В solo-флоте не влияет (один тип цели у защитника) |

### 20.3 Выбор: RF×45 vs cap=50

**RF×45 преимущества**:
- Чистый ребаланс, без новой game-механики.
- Не ломает 1v1 матрицу (RF активируется только когда Cruiser стреляет
  в Lancer-mass; при equal-cost дуэли обычный atk применяется).
- Согласуется с принципом «counter через RF» (как Cruiser×6 vs LF,
  Bomber×20 vs RL).
- 1 число в YAML.

**Cap=50 преимущества**:
- Гарантия от любого spam (даже если игрок придумает обход).
- Простой rollback.

**Выбран RF×45** как минимально инвазивный.

### 20.4 27-W' внедрение

- `configs/rapidfire.yml`: `33→102: 35 → 45`.
- `configs/ships.yml`: удалено `max_per_planet: 50`.
- `backend/internal/config/catalog.go`: удалено поле `MaxPerPlanet` из
  `ShipSpec`.
- `backend/internal/shipyard/service.go`: удалена cap-проверка в
  `Enqueue` и `ErrPlanetCapExceeded`.
- `backend/internal/shipyard/handler.go`: удалён маппинг
  `ErrPlanetCapExceeded` → HTTP 400.

### 20.5 Регрессия

Все ключевые сценарии стабильны:
- `lancer-vs-mixed`: draw 100%, atk loss 96%, def loss 81%, exchange 0.10.
- `lancer-bs-combo-vs-mixed`: draw, exchange 0.61 (почти как baseline 0.73).
- `lancer-vs-cruiser` 1v1: defender wins (Lancer всё равно проигрывает).
- Матрица 1v1: идентична Variant Б (cap не влиял на 1v1).
- Группы: lancer-fleet vs shadow-fleet 11.92, lancer-fleet vs lite 0.16
  — стабильны.
