...
> **Итог**: фикс применён в одной центральной точке `sqlInsert()` +
> 9 точечных правках прямых `Core::getDB()->insert_id()` без обёртки.
> Smoke подтвердил: при провале INSERT (FK violation на пустой
> `na_sim_planet`) `lastInsertId()` действительно указывает на PK
> от ПРЕДЫДУЩЕГО успешного INSERT (тот же бой = `assaultid=10`),
> подтверждая корневую причину ранее неуловимого бага. С фиксом
> `addParticipant` early-return'ится при `rc===false`, каскадного
> мусора в `sim_fleet2assault` больше не появляется. Happy-path
> (с восстановленным seed'ом) работает без регрессий.
