...
**Зависимости**: блокируется планом 64 (alien-юниты в legacy.yaml).
**Связанные документы**:
...
(legacy-режим): реализовать оставшиеся EVENT_ALIEN_*, перенести
полный AI-движок (`AlienAI.class.php` 1127 строк → ~800 строк Go).

Применимо для legacy-вселенной (legacy01) под флагом, в modern
вселенных (uni01/uni02) AlienAI остаётся в текущем урезанном виде
плана 15.
...
- Алгоритм **`generateFleet()`** — target_power, итеративное
  добавление кораблей.
- 5 алиен-кораблей UNIT_A_1..UNIT_A_5 в `legacy.yaml` (план 64
  уже добавил).
- Множитель «четверг» ×5 / ×1.5..2.0 — вынести в `legacy.yaml`
  как параметр.
...
- Не дублируем в modern-вселенных. Все новые механики под флагом
  `universe.balance_profile = legacy` ИЛИ отдельным флагом
  `universe.alien_ai_full_parity`.
...
- Алиен-юниты в legacy.yaml: `alien_unit_1`..`_5` (snake_case),
  не `UNIT_A_*`.
