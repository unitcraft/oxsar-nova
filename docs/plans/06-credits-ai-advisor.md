...
### F.4 Автопилот (rule-based, без LLM)

Вынесено в подплан [06.1: Автопилот](06.1-autopilot.md).

Краткое: детерминированная система, которая знает всё состояние игрока и все
внутренние формулы, предлагает топ-3 оптимальных хода, выполняет выбранный
по подтверждению игрока. Расчёт в воркере (`KindAutopilotAdvise`), результат
доставляется через WebSocket. Представлен как «модель» в виджете советника
наряду с LLM-вариантами (Haiku / Sonnet / Opus).
