...
|---------------------|-------------------|-------------------|
| `<input type="range">` ползунок + пресеты 0/25/50/75/100 | `<select>` шаг 10% + `<input type="number">` | Низкий |
| Автосохранение по onMouseUp (без кнопки COMMIT) | Кнопка «Сохранить» (ручное) | Низкий |
| Убрать кнопку «Назад» (навигация через меню) | Кнопка «Назад» присутствует | Низкий |
| Убрать строку «Еженедельное производство» | ✅ Убрана | — |
| Убрать select выпадающий список шаг 10% | Select присутствует | Низкий |
| Убрать input text для ручного ввода % | Input number присутствует | Низкий |

Текущий вариант функционально полон — расхождения касаются только UX-деталей.
