...
- автоматическая регистрация фискального чека через API ЮKassa
  (для применимого налогового режима оператора) — без ручной
  выписки чеков;
...
### 2. Фискальный чек через API ЮKassa

В запросе `CreatePayment` передавать объект `receipt` с:
- `customer.email` (или `customer.phone`) — обязательно для фискального
  чека;
- `items[]` с описанием "Игровые кредиты, пакет N", `quantity`,
  `amount.value`, `vat_code` в соответствии с применимым налоговым
  режимом оператора, `payment_subject: service`,
  `payment_mode: full_payment`.

ЮKassa автоматически регистрирует чек в ФНС при правильно заполненном
`receipt`. Проверка после первой реальной транзакции в режиме
тест-магазина: чек должен появиться в учётной системе оператора
в течение суток.
...
- автоматический фискальный чек через API ЮKassa;
...
- привязка к учётной системе оператора (если применимо для выбранного
  налогового режима — например, выдача разрешения ЮKassa на
  регистрацию фискальных чеков от имени оператора);
...
- Проверить, что в режиме реального магазина фискальный чек
  регистрируется в учётной системе оператора.
...
- Обновить `docs/project-creation.txt`: "план 42: ЮKassa как основной
  платёжный шлюз с автоматическим фискальным чеком".
