...
**Дата**: 2026-04-27
**Статус**: Активный
**Зависимости**: **План 51** (rename), **План 52** (RBAC), **План 53** (admin-frontend) должны быть выполнены.
...
1. **Frontend** (`portal/frontend` или где «Пополнить кредиты»):
...
- В существующих тестах: добавить case «limit reached → BuildPayURL
  отказывает».
- Frontend (где «Пополнить кредиты»): отдельный endpoint
