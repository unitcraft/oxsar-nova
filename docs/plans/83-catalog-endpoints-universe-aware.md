...
Все 4 catalog-endpoints получают universe-параметр. Без `?universe`
— fallback к JWT-context (current planet's universe) или modern-
default.
...
**Особенности по endpoint'ам:**

1. **`/api/buildings/catalog/{type}`** (наибольший объём изменений):
   - Params из `configs/buildings.yml` + override
     `configs/balance/origin.yaml` (план 64 deep-merge).
   - Формулы — `internal/economy/formulas.go` (modern) vs
     `internal/origin/economy/` (origin):
     `MetalMineProduction`, `CostAtLevel`, `BuildTime` и др.
   - Pre-computed таблица для уровней 1, 5, 10, 20, max — пересчёт
     по выбранным формулам.

2. **`/api/units/catalog/{type}`**:
   - Params из `configs/units.yml` + override.
   - Формулы статов (атака, защита, щит) с tech-бонусами зависят
     от вселенной (modern vs origin могут иметь разные множители).
   - Pre-computed статы для нескольких уровней техники.

3. **`/api/research/tree`**:
   - Params из `configs/research.yml` + override.
   - Формула стоимости research per-level — может различаться.
   - Структура графа `{nodes, edges}` обычно одинакова между
     вселенными (зависимости одинаковые), но **значения cost
     зависят от формулы**.

4. **`/api/artefacts/catalog/{type}`** (наименьший объём):
   - Параметры в `configs/artefacts.yml` — это в основном
     статические данные (имя, описание, эффект, lifetime), без
     сложных формул.
   - Per-universe override актуален в основном для alien-артефактов
     (`internal/origin/alien/` — артефакты origin-вселенной которые
     не существуют в modern).
   - Universe-routing для artefacts проще: фильтрация каталога
     («какие артефакты доступны в этой вселенной»), без пересчёта
     формул.
   - Если artefacts полностью совпадают между вселенными по
     статическим данным — universe-routing для artefacts может
     быть **отложен** в follow-up план без вреда.
