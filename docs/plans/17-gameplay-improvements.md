...
### B2. Unknown (id=13) — потеря времени с намёком на тайну

**Суть**: флот вернулся без объяснений на 2-4ч позже запланированного.  
Реализация: создаём новый `KindFleetReturn` с `fire_at += random(2h,4h)`, сообщение «Флот вернулся. Экипаж молчит об увиденном.»

---

## Блок C: HOLDING_AI — подарки за плен (Приоритет: M)
...
## Блок D: Ежедневные задания (Daily Quests) (Приоритет: M)
...
## Блок E: Категориальные рейтинги (Приоритет: L)
...
### G3. Сравнение в BattleSimulator

В BattleSimulatorScreen: кнопка «Сохранить состав» (localStorage).  
Два слота: «Мой состав» vs «Альтернатива» — side-by-side результат.  
Чисто frontend, без API.

### G4. Fleet tracker в Galaxy

В GalaxyScreen: на клетке с координатами, куда летит мой флот, — иконка ✈️.  
Данные уже есть в `GET /api/fleet/movement`. Пересечение с galaxy coords — на фронте.

---

## Блок H: Улучшения боевого движка (Приоритет: M/L)
