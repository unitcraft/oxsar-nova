...
- **Что делает игрок**: создаёт лоты артефактов на продажу за
  оксариты, покупает чужие лоты, устанавливает TTL (1ч-7 дней),
  история торговли
- **Backend**: ✅ план 68
- **Аналог в nova**: `artmarket` (single-item P2P) сосуществует с
  биржей (пакетный P2P)
- **Кандидат на добавление**: ⭐ да, ключевая legacy-фича
- **Связь с origin**: обязательно для legacy01
- **Объём UI**: 3 экрана — список, детали лота, создание; ~1-2 недели
  (план 76 / спринт 5 плана 72)
...
**Статус (2026-04-28, план 71)**: Ф.1 завершена. Помечены
✅ — реализованы; ⏳ — отложены (Приоритет L); ✱ — частично
покрыто как побочный эффект.
