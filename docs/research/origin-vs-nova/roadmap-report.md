...
#### R12 — переиспользование nova-строк (важно для серии 64-74)

В nova **уже есть** значительный i18n-bundle (multiple переводов
для существующих фич: бой, экономика, дипломатия, чат, уведомления).
**Идентификаторы строк отличаются** от legacy-PHP origin
(в legacy-PHP — таблица `na_phrases` с PHP-ключами вроде
`MSG_INV_PASS`, в nova — snake_case namespace-ключи).

**Правило приоритетов при работе с серией 64-74:**

1. **Сначала ищи существующий ключ в nova-bundle.** Если в
   nova есть `auth.invalid_password` со смыслом «неверный пароль» —
   **используй его**, не заводи дубликат типа `auth.password_invalid`
   или `login.wrong_password`.
2. **Совпало логически + по группе** → переиспользовать. Группы:
   `auth`, `fleet`, `alliance`, `chat`, `mail`, `building`,
   `research`, `combat`, `expedition`, `error.common` и т.д.
3. **Логически совпадает, но разные группы** — оценить, стоит ли
   обобщить. Например, `error.common.not_enough_resources` vs
   `building.queue.not_enough_resources` — если строка одинаковая
   везде, использовать `error.common.*`.
4. **Логически совпадает, но смысл всё-таки тоньше** (например,
   «не хватает энергии для здания» vs «не хватает энергии для
   движения флота») — заводить отдельный ключ. Лучше две точные
   строки, чем одна расплывчатая.
5. **Совсем нет аналога** — заводить **новый** ключ в подходящей
   группе, по конвенциям nova (snake_case, namespace).

**❌ НЕ переносим** идентификаторы legacy-PHP-фраз:
- `na_phrases.phrase_key = 'MSG_INV_PASS'` → не превращается в
  `msg_inv_pass`. Ищем существующий nova-ключ или заводим новый
  по nova-конвенции.
- Тексты на русском — переиспользуем как **значения**, ключи —
  всегда nova-стиль.

**Применение к планам серии 64-74:**

- При написании нового handler'а / endpoint'а / UI-компонента —
  агент **обязан** сделать grep по `projects/game-nova/configs/i18n/`
  на похожие ключи. Если нашёл — использует. Если не нашёл —
  заводит новый.
- В коммит-сообщениях планов 64-74 — кратко указывать, **сколько
  i18n-ключей переиспользовано** vs **создано новых**. Это
  индикатор качества работы.

**Особо критично для:**
- **План 72** (origin-фронт) — много user-facing строк (50 экранов).
  Без переиспользования получится дубликат всего nova-bundle.
- **План 67** (alliance) — диалоги дипломатии, статусы.
- **План 68** (биржа) — фильтры, статусы лотов, ошибки.
- **План 70** (achievements, отложен) — описания ачивок.
- **План 71** (UX-микрологика) — tooltip'ы и подсказки.
...
✅ **ЗАВЕРШЁН 2026-04-28** (3 коммита, ~1500 строк Go + 395 строк
YAML, разблокирует планы 65/66/69).
