...
#### Что СЧИТАЕТСЯ упрощением (требует обоснования в simplifications.md)

- ❌ Архитектурное отклонение от плана / промпта (например, «promt
  говорил middleware, я сделал inline-guards»). Допустимо при наличии
  серьёзной причины + плана возврата.
- ❌ Не реализованная фича из плана с обоснованием «отложено до Ф.X».
- ❌ Снижение покрытия тестов / метрик / observability в обмен на
  скорость.

#### Что ПРОПУСК (не trade-off, считается багом — добавить и не отмечать)

Эти вещи **тривиально добавляются** (5-30 строк) и **не зависят** от
другой работы. Пропуск не оправдан «низкочастотный/высокочастотный/
низкокритичный» — это **просто пропуск**, не trade-off:

- 🚫 **R8 Prometheus метрики** — counter+histogram для нового
  handler/endpoint. 5-10 строк, всегда. Аргумент «низкочастотный
  endpoint» НЕ принимается — observability нужна именно для
  редких операций (если они начнут падать, ты не узнаешь без
  метрики).
- 🚫 **R9 Idempotency-Key** для мутирующего endpoint'а — middleware
  у нас уже есть, подключение на endpoint = 1-2 строки.
- 🚫 **R12 i18n** — хардкод строки в Go-handler. Ключ + Tr() = те же
  2-3 строки.
- 🚫 **R10 universe_id** в новой таблице с per-universe данными —
  это поле из 10 символов в DDL.
- 🚫 **R3 slog с trace_id** в новом handler'е — стандартный паттерн.
- 🚫 **DB-level CHECK** на размер VARCHAR/TEXT, когда есть handler-level
  лимит — это **defense-in-depth**, тривиально, защищает от обхода
  handler'а через bg-jobs или ручные SQL.
- 🚫 **NOT NULL** на колонке, которая по семантике не nullable.
- 🚫 **FK constraint** на ссылку (с ON DELETE действием) для новой
  колонки.
- 🚫 **Index** на FK или часто-фильтруемой колонке.

**Если агент зафиксировал что-то из списка 🚫 в simplifications.md** —
это **сигнал что обоснование слабое**. Должен быть пере-сделан, не
оставлен как trade-off.

#### Что НЕ упрощение (тривиальные различия от плана)

- ✅ Имя поля в БД отличается от плана (план говорил `notes`, агент
  сделал `content` в существующей таблице `user_notepad` — это
  даже лучше, нет дубликата).
- ✅ Лимит вместо «16KB» поставил «50KB» — если решение обосновано
  (handler уже работает с этим лимитом, миграция legacy не нужна).
- ✅ 7 permissions вместо 6 — миграция уже включала больше.
- ✅ Endpoint существовал — агент задокументировал в OpenAPI задним
  числом, не пере-создал.

Это всё **здоровая инженерная адаптация к реальности**, не упрощения.
