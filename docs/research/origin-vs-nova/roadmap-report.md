...
### План 63: legacy.yaml + per-universe balance loading
...
### План 64: Расширение event-loop (legacy события)
...
### План 65: AlienAI до полного паритета с legacy
...
### План 66: Расширение alliance-системы
...
### План 67: Биржа артефактов (Exchange/Stock)
...
### План 68: Расширение domain-полей в nova
...
### План 69: Achievements расширение (legacy + общий движок)
...
**Объём**: 1-2 недели.
**Зависит от**: goal engine (план 65 уже есть).

---

### План 70: UX-микрологика origin → nova-frontend
...
### План 71: Origin-фронт — pixel-perfect клон (главный)
...
**Зависит от**: 63, 64, 65, 66, 67, 68, 69, 70 (вся backend-готовность);
57 (mail/TipTap). Может быть **частично** запущен раньше — экраны,
backend которых уже готов.

---

### План 72: Screenshot-diff CI (Playwright + visual regression)
...
**Закрывает**: пороги качества плана 71 (паритет визуала).

**Объём**: 2 недели.
**Зависит от**: 71 (хотя бы первые экраны).

---

### План 73: legacy01 deploy + DNS + config
...
70 (UX-микрологика) ───────┤
                           ▼
57 (mail-service, готов)──→ 71 (origin-фронт) ──→ 72 (CI) ──→ 73 (deploy)
                            ▲
...
| pixel-perfect клон будет «уплывать» при правках UI | Screenshot-diff CI (план 72) — баг сразу виден |
| Шрифты/иконки origin не попадают под лицензию | Аудит лицензий ДО плана 71. Замена несовместимых на open-source аналоги |
...
| Турниры (D-031, U-002) в первой итерации | отдельный план **после** плана 73 | Не блокирует ремастер |
...
2. **Точные цвета палитры origin** — взять из `style.css`
   программно или вручную. План 71 решит.
3. **Шрифты в legacy** — какие именно, лицензии. План 71 решит
   аудитом.
4. **Какие именно из 100+ ачивок origin переносить** — нужен
   отбор приоритетных vs «исторических». План 69 решит.
5. **Поведение balance_profile в multi-universe DB** — каждая
   вселенная в одной DB или отдельные shard'ы? Уточнить с
   архитектором перед планом 63.
6. **Лицензии иконок origin** — критичный блокер. Аудит до плана 71.
7. **Чат фан-аут с TipTap-payload** — план 32 готов, но
   протестировать на ~100 одновременных пользователей до плана 71.
...
```
Месяц 1-2:   [63 legacy.yaml]──┐
                                │
Месяц 2-3:   [64 event-loop]   │  [66 alliance]   [67 exchange]
                                │
Месяц 3-4:   [65 AlienAI]──────┤  [69 achievements]
                                │
Месяц 4-5:   [68 domain fields]┘  [70 UX-микрологика]
                                
Месяц 5-9:   [71 origin-фронт — pixel-perfect клон]
              (5 spring'ов по 3-4 недели каждый)
                                
Месяц 9-10:  [72 CI screenshot-diff]
                                
Месяц 10:    [73 legacy01 deploy]
```
...
| D-026 (формула DSL источник) | формула | 63 |
| D-027 (RF алиенов) | формула | 63 + 65 |
| D-028 (спец-юниты) | формула/домен | 63 |
| D-029 (температура водорода) | формула | 63 |
| D-030 (charge экспоненты) | формула | 63 |
| D-031 (TOURNAMENT) | event-loop/механика | **отказ** в первой итерации (Часть V) |
| D-032 (TELEPORT_PLANET) | event-loop/механика | 64 |
| D-033 (TEMP_PLANET) | event-loop | 64 верификация (вероятно ✅) |
| D-034 (RUN_SIM_ASSAULT) | event-loop | **отказ** (Часть V) |
| D-035 (DELIVERY_ARTEFACTS) | event-loop/механика | 64 |
| D-036 (alien chains) | event-loop/механика | 65 |
| D-037 (ATTACK_DESTROY_BUILDING) | event-loop/механика | 64 |
| D-038 (ALIEN_ATTACK_CUSTOM) | event-loop | 53 (admin-bff) |
| D-039 (биржа артефактов) | api/механика | 67 |
| D-040 (передача лидерства) | api | 66 |
| D-041 (3 описания альянса) | api/домен | 66 |
| D-042 (global mail альянса) | api/механика | 66 (после 57) |
| D-043 (Phalanx) | api | 64 верификация |
| D-044 (ResTransferStats) | api | 66 |
| D-045 (ExchangeOpts) | api/механика | (опционально, после 73) |
| D-046 (artefact-image PHP-GD) | assets | 71 |
