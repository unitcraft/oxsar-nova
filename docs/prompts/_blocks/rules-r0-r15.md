...
🚫 ПРОПУСК (повтор для важности — не trade-off, **сделать полностью**):
- Mock-данные в production-frontend «потому что endpoint'а нет в
  backend» — это **классическое нарушение R15**. Отсутствующий
  endpoint в frontend-сессии = расширение backend в той же сессии
  (обычно 50-200 строк Go), не TODO + simplifications.md.
- TODO-комментарии в production-коде с отсроченной реализацией.
- Hardcoded fallback-значения вместо чтения из конфига.
- «Сделаем после публичного запуска» для функциональности которую
  игрок видит и без которой UI неполный.
```
