...
### [UI] ResourceScreen: FactorInput — select+number вместо slider+пресетов
- **Где**: `frontend/src/features/resource/ResourceScreen.tsx` — `FactorInput`
- **Что упрощено**: план требовал `<input type="range">` + кнопки-пресеты 0/25/50/75/100 и автосохранение по `onMouseUp`. Реализован `<select>` с шагом 10% + `<input type="number">` + ручная кнопка «Сохранить».
- **Почему**: компонент переписывался «с нуля» после удаления старой версии; выбран быстрый рабочий вариант.
- **Как чинить**: заменить `FactorInput` на range+пресеты, убрать кнопки «Сохранить» и «Назад», добавить автосохранение по `onMouseUp`/`onTouchEnd`.
- **Приоритет**: L
