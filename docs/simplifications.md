...
### [P72.1.33.PACK_ARTEFACTS_DEFERRED] Pack building/research в артефакт — отложено

**Создано 2026-05-01** ([план 72.1.33](plans/72.1.33-building-info-cont.md)).
Отложено сознательно — требует отдельного крупного плана инфраструктуры.

- **Где**: legacy `BuildingInfo::packCurrentConstruction` +
  `Artefact::canPackBuilding` / `canPackResearch`.
- **Что**: legacy позволяет «упаковать» здание/исследование игрока в
  артефакт типа `ARTEFACT_PACKED_BUILDING` / `ARTEFACT_PACKED_RESEARCH`,
  если у игрока на этой планете уже активен packing-артефакт
  (`ARTEFACT_PACKING_BUILDING` с `active=0`). Packed-артефакт
  можно потом активировать на другой планете для мгновенного
  получения уровня. Уровень здания при упаковке понижается до
  `level - added` (учитываются ранее «added» через packed).
- **В origin**: оба endpoint'а отсутствуют. UI BuildingInfo не
  показывает кнопок «упаковать здание/исследование».
- **Почему отложено**: pack/packed — это два разных типа артефактов,
  для них нет state в `artefacts_user` (нет `active`/`delay_eventid`/
  `times_left`/`use_duration` колонок в схеме nova) и нет
  catalog-описаний в `configs/artefacts.yml`. Реализация требует:
  миграции `artefacts_user` (расширение state), 2 новых типа
  артефактов в catalog с requirements, `ArtefactActivate`-сервиса
  (с проверкой `level > added`, INSERT в packed-таблицу), event
  delay для packed-артефактов, UI в BuildingInfo + ArtefactInfo.
  Это отдельный план уровня §6 (~500 LOC), не cleanup task для
  72.1.33.
- **Trade-off**: minor. Pack-артефакты — premium-механика, в MVP
  игроки прекрасно обходятся обычной стройкой/исследованиями.
  Игроки не теряют ресурсы, не получают/теряют уровни.
- **Как чинить**: новый план «Artefact pack/packed system» —
  миграция, 2 типа артефактов (packing/packed) для buildings и
  research = 4 типа, расширение state в artefacts_user (`active`,
  `delay_eventid`, `times_left`, `lifetime_eventid`), сервис
  активации с requirements, UI кнопки.
- **Приоритет**: L (post-MVP, требует отдельного плана). Не
  блокирует gameplay.
