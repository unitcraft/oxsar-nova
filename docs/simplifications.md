...
> **📅 Напоминание (2026-05-17, ~2 недели после 72.1.56):** провести
> re-audit этого файла на предмет накопления новых записей с момента
> 2026-05-03. План действий:
> 1. `git log --since="2026-05-03" -- docs/simplifications.md` — что
>    добавлено за период.
> 2. `grep -n "^### \[" docs/simplifications.md | grep -vE
>    "ЗАКРЫТО|CLOSED|✅|закрыто"` — все открытые записи; сравнить
>    с состоянием на 2026-05-03 (было ~70 открытых после 72.1.56).
> 3. Категоризировать новые записи: A — verify-handsweep
>    (ложно-открытые), B — реальные 1:1 фиксы, C — by-design,
>    D — L-приоритет.
> 4. Если есть кандидаты Группы A или B на 1-3 часа каждый — сказать
>    Claude и запустить новую итерацию (паттерн как 72.1.56).
> 5. Если ничего нового — удалить этот блок и идти дальше.
>
> _Этот блок добавлен 2026-05-03 как замена session-only cron-задаче
> (durable не поддерживается в текущей сборке Claude Code)._
...
### [M4.x] attacker_* поля юнитов не используются в бою — ЗАКРЫТО
- **Статус**: ЗАКРЫТО 2026-05-03 планом 72.1.56 B11 (legacy 1:1).
- **Где закрыто**:
  - `internal/config/catalog.go`: добавлен `AttackerOverrides{Front,
    Attack, Shield, Shell}`, optional `Attacker *AttackerOverrides` в
    `ShipSpec` и `DefenseSpec`. Ballistics/Masking опущены — они
    давно tech-level (ADR-0015), per-юнит не имеют смысла.
  - `configs/ships.yml`: `death_star.front=10` + `attacker.front=9`,
    `unit_a_screen.front=15` + `attacker.front=16` (1:1 с
    `na_ship_datasheet`).
  - `internal/fleet/attack.go::stacksToBattleUnits` принимает
    `isAttacker bool`. Также починен Front (был захардкожен 0!).
    Все callers обновлены: attack.go (atk=true, def=false),
    acs_attack.go (defenders=false, fleets=true), expedition.go (PvE
    игрок=attacker).
  - `internal/alien/helpers.go::stacksToBattleUnits` зеркально
    обновлён.
- **Тест**: `TestStacksToBattleUnits_AttackerOverrides` проверяет
  Deathstar Front=9 при role=attacker и =10 при defender; cruiser без
  overrides сохраняет 7 в обеих ролях.
- **Регрессия Find=0 устранена** — все юниты теперь корректно несут
  Front в бой.
- **Приоритет**: closed.
...
### [65-Ф.1] Нет публичного API для demolish — только handler — ЗАКРЫТО
- **Статус**: ЗАКРЫТО (verify-handsweep 2026-05-03 при 72.1.56). Запись
  была устаревшая — полный demolish-stack уже реализован планом 72.1.33:
  - `building/service.go::EnqueueDemolish(ctx, userID, planetID, unitID)`:
    cost = `(1/spec.Demolish) × cost_at_current_level`,
    duration = `build_time × 0.5`, INSERT construction_queue (kind='demolish')
    + INSERT event KindDemolishConstruction.
  - `building/handler.go::Demolish` →
    `POST /api/planets/{id}/buildings/{unitId}/demolish`.
  - Route в `cmd/server/main.go:521` с `idemMW.Wrap` (Idempotency-Key).
  - Frontend `frontends/origin/src/api/buildings.ts::demolishBuilding`
    + ConstructionsScreen кнопка «⚒ Снести» с ConfirmDialog
    (destructive=true).
- **Приоритет**: closed.
...
- **Что было упрощено**: legacy `Alliance.class.php::updateAllyPrefs`
  (L.979-1015) — 6 полей не реализованы: `logo, homepage,
  foundername, showmember, showhomepage, memberlistsort`.
- **Что закрыто** (1:1 паритет):
  - Миграция [0096_alliance_prefs.sql](../projects/game-nova/migrations/0096_alliance_prefs.sql):
    +6 колонок в `alliances` (logo/homepage/foundername nullable text;
    show_member/show_homepage bool default true; memberlist_sort
    smallint default 0).
  - Backend `Service.UpdatePrefs` + `UpdatePrefsInput` с валидацией:
    logo/homepage URL regex (legacy `isValidImageURL`/`isValidURL`),
    длина ≤128, foundername ≤64, memberlist_sort 0..4. Sentinel
    errors: `ErrLogoTooLong/Invalid`, `ErrHomepageTooLong/Invalid`,
    `ErrFoundernameTooLong`, `ErrMemberlistSortRange`.
  - Backend `Handler.UpdatePrefs` + route `PATCH /api/alliances/{id}/prefs`
    (Idempotency-Key поддерживается через `idempotency.FromRequest`).
  - OpenAPI: PATCH endpoint описан + Alliance schema расширена.
  - Frontend `AllianceManageScreen.tsx`: форма с 6 полями (URL,
    text, 2 checkbox, select sortMode 0..4), `updateAlliancePrefs`
    mutation.
  - Frontend `AlliancePageScreen.tsx`: показ logo/homepage/foundername
    на public-странице альянса при `show_homepage=true`.
  - i18n keys: `alliance.preferences/logoUrl/homepageUrl/showHomepage/
    foundername/showMember/memberlistSort/sort{Rank,Name,Points,Joined,LastSeen}`.
...
### [P88.2] Push-уведомления офицеров подставляют английский id вместо имени — ЗАКРЫТО
- **Статус**: ЗАКРЫТО 2026-05-03 планом 72.1.56 Task A3.
- **Где закрыто**: i18n группа `officerName: { ADMIRAL, GEOLOGIST,
  ENGINEER, MERCHANT }` (ключи совпадают с officer_defs.key); helper
  `Service.officerName(key)` резолвит OfficerKey→русское имя через
  `bundle.Tr(officerName, key)`; subject ключи переформулированы
  как «Подписка на офицера продлена: {{name}}» / «Срок подписки на
  офицера истёк: {{name}}» (без слова "Officer").
- На выходе игрок видит «Срок подписки на офицера истёк: Инженер»
  вместо «Officer engineer истёк».
- **Приоритет**: closed.

### [P88.3] `creditRepairVIP.body` подставляет английский mode-id

- **Где**: [projects/game-nova/configs/i18n/ru.yml:682](../projects/game-nova/configs/i18n/ru.yml#L682) —
  `Списано {{credits}} кредитов за мгновенный старт {{mode}} ({{quantity}} ед.).`
- **Что**: плейсхолдер `{{mode}}` подставляет, скорее всего,
  английский id операции типа `repair` / `disassemble` (по контексту
  legacy `MSG_CREDIT_FOR_REPAIR_VIP` / `..._FOR_DISASSEMBLE_VIP`,
  план 72.1.25). Игрок увидит «за мгновенный старт repair» —
  английщина в видимом тексте.
- **Почему не правится в плане 88**: правка только текста ничего не
  решит — нужно либо резолвить `mode` → русское слово на бэкенде
  («ремонт» / «разборка»), либо разделить ключ на два разных
  (`creditRepairVIP.body` для repair, `creditDisassembleVIP.body`
  для disassemble) с уже встроенными русскими словами в текст.
- **Trade-off**: minor. Текст работоспособен, но выглядит как баг.
- **Как чинить**: либо (a) добавить блок `vipMode: { repair:
  "ремонт", disassemble: "разборка" }` в i18n + резолв в коде;
  либо (b) разделить ключи без подстановки.
- **Приоритет**: M. Аналогично закрытому P88.2 — типовая задача,
  ~10-15 минут работы.

### [P88.4] `msgExpireArtefact` и `msgDeactivateArtefact` дублируют логически разные события

- **Где**: [projects/game-nova/configs/i18n/ru.yml:632,636](../projects/game-nova/configs/i18n/ru.yml#L632).
- **Что**: до пачки #4 оба ключа содержали идентичный текст
  «Выключился артефакт {@name} на {@coords}». В пачке #4 я
  переписал `msgExpireArtefact` как «Срок действия артефакта
  истёк» (оригинальная семантика — по таймауту), а
  `msgDeactivateArtefact` оставил как «Деактивирован» (выключен
  пользователем). Если бэкенд триггерит **только** один из этих
  ключей в обоих кейсах — это баг разработчика, и игрок будет
  видеть только один вариант.
- **Почему оставлено в текстах**: правильное разделение
  семантики — правильный ход, даже если код пока триггерит один
  ключ. Когда баг будет исправлен, тексты уже готовы.
- **Как проверить**: grep `msgExpireArtefact|msgDeactivateArtefact`
  по backend, посмотреть в каких сценариях каждый отправляется.
  Если оба триггерятся — закрыть как closed; если только один —
  завести bug-fix задачу.
- **Приоритет**: L. Текст не сломан, нужно проверить логику.
