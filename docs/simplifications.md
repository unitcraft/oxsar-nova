...
### [Tutorial] Нет наград кроме кредитов
- **Где**: `internal/tutorial/service.go::rewardPerStep`.
- **Что**: каждый шаг даёт только +10 кредитов. В oxsar2 давались ресурсы
  (metal/silicon), иногда корабли.
- **Почему**: минимальная reward-система без усложнения транзакционной логики.
- **Как чинить**: расширить advanceAndReward для выдачи ресурсов на стартовую планету.
- **Приоритет**: L.
