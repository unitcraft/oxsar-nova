...
**UPD 2026-04-24 (второй проход, после подключения к legacy БД)**: быстрые
рекомендации применены — см. коммит ниже. Всё критичное починено. Что
осталось:
- `defense_factory` (id=101) объявлен в `units.yml`, но отсутствует
  в `buildings.yml`. Это здание из легаси, у нас не реализовано.
- Planetary shields (`small_planet_shield` 354, `large_planet_shield` 355)
  — легаси имеет полные статы + требования, но эти механики у нас не
  реализованы в коде (нет battle-эффекта, нет UI). Нужен ADR если
  захотим добавить.
- Ракеты в `units.yml defense`-группе (51, 52): фактически работают
  как fleet через ships.yml. Перенос в fleet-группу — ADR.
