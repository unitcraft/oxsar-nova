...
- Для РФ-аудитории (наш случай): **Selectel** ~600-900 ₽/мес или
  **Timeweb Cloud** ~400-600 ₽/мес. Подробности и pros/cons — в разделе
...
- Для РФ-аудитории: **Selectel** ~1500-2200 ₽/мес или **Timeweb Cloud**
  пропорционально дешевле. Апгрейд внутри той же панели — без миграции.
...
**Рекомендация** (для РФ-аудитории):
- **App**: Selectel облачный сервер 4 vCPU / 8 GB.
- **DB**: Selectel **с dedicated vCPU** (выделенные ядра критичны для
  PG под нагрузкой, burstable не подходят) либо managed PostgreSQL
  Selectel — со снятием операционной нагрузки backup'ов и репликации.
- **Итого ~5000-8000 ₽/мес** в зависимости от того, dedicated VPS или
  managed PG.
...
**Рекомендация** (для РФ-аудитории):
- Selectel: app × 4 (4 vCPU/8 GB) + worker × 4 (2 vCPU/4 GB) + DB primary
  (8 vCPU/32 GB dedicated) + replica (4 vCPU/16 GB) + Redis + LB.
  Ориентировочно **~25 000-35 000 ₽/мес**, точная цифра зависит от
  managed-обвязки и резервирования.
- На этом масштабе уже имеет смысл считать совокупную стоимость с
  managed PG Selectel — снимает работу по бэкапам, реплике, мажорным
  апгрейдам.
...
| Профиль | DAU | CCU | App | DB | Redis | Цена (РФ-провайдер) | Готовность кода |
|---|---|---|---|---|---|---|---|
| 1. Soft launch | < 500 | < 50 | 2 vCPU / 4 GB | вместе | вместе | **400-900 ₽/мес** | Текущее состояние ок |
| 2. Ранняя | 500-2k | 40-160 | 4 vCPU / 8 GB | вместе | вместе | **1500-2500 ₽/мес** | + бэкапы, + мониторинг |
| 3. Рост | 2k-10k | 160-800 | 4 vCPU / 8 GB | 4 vCPU / 16 GB | share app | **5000-8000 ₽/мес** | + score-incremental, + res_log TTL, + фазы 1-2 плана 09 |
| 4. Масштаб | 10k-50k | 800-4k | 4 × (4/8) + 4 × (2/4) | 8/32 + replica | 2/4 | **25 000-35 000 ₽/мес** | + фаза 3 плана 09, + PgBouncer, + chat Redis pub/sub |
| 5. Высокая | 50k-200k | 4k-16k | N × app | PG shards | cluster | **80 000-200 000 ₽/мес** | Редизайн: sharding, CDN, k8s |
...
Аудитория проекта — **РФ-сегмент** (русскоязычные игроки). Это решающий
фильтр: EU-провайдеры технически непригодны (см. блок «РФ-специфика»
ниже), независимо от их цены.

| Провайдер | Плюсы | Минусы | Для профилей |
|---|---|---|---|
| **Selectel** (РФ) | Tier III ДЦ Москва/СПб, SLA 99.98%, managed PG, базовая DDoS-защита включена | в ~2× дороже Hetzner, тарифы быстро накручиваются доп.услугами | 1-5 |
| **Timeweb Cloud** (РФ) | дешевле Selectel на 20-30%, простая панель | вечерние просадки скорости, инциденты с DNS, разово выдавали дублирующийся IP | 1-3 |
| **Yandex Cloud** (РФ) | managed PG/Redis, k8s | в 4-5× дороже + **платный egress** (10 GB free, дальше за GB) — несовместимо с 20 TB/мес на DAU > 500 | 4-5 (только когда нужен managed) |
| **Aeza** (РФ/EU) | дёшево, заявленная DDoS-защита до 2 Тбит/с | в 2025 несколько массовых падений по локациям, поддержка отвечает по 8 ч | избегать для 24/7 MMO |
| **Hostkey, VDSina, FirstVDS** (РФ) | дёшево на момент покупки | резкие односторонние повышения цен в 2024-2025 (Hostkey: 440 → 1100 ₽ за полгода) | избегать для долгого проекта |
| **Hetzner Cloud** (EU) | флагман low-cost, NVMe + 20 TB | оплата с РФ-карты не работает; РКН периодически блокирует подсети — **игроки из РФ не зайдут** | непригоден для РФ-аудитории |
| **netcup, Contabo** (EU) | очень дёшево | те же проблемы оплаты + IOPS-тротлинг (Contabo) / троттлинг «выделенных» ядер (netcup VPS) | непригоден |

**Личная рекомендация для старта**: **Selectel** для предсказуемости
(дороже, но без сюрпризов), **Timeweb Cloud** если бюджет жёстче и
готовы мониторить просадки. С Профиля 3 — Selectel managed PG.

### РФ-специфика (важно для физлица)

- **Оплата EU-провайдеров (Hetzner/netcup/Contabo)** с российской карты
  не работает с 2022, в 2025-2026 ситуация не изменилась. Рабочие
  способы — виртуальные карты иностранных банков или USDT через
  посредников. Для физлица это лишний риск и комиссия.
- **Блокировки РКН**: в 2024-2025 РКН включил Hetzner Online GmbH в
  список «приземления» и периодически блокировал отдельные подсети для
  пользователей из РФ. Для проекта с РФ-аудиторией это не косметика —
  игроки физически не зайдут без VPN.
- **Cloudflare Free** в 2025-2026 в РФ деградировал (TLS ECH ограничения
  с конца 2024, замедления у ряда операторов). Для РФ-сегмента полагаться
  на встроенную DDoS-защиту провайдера: у Selectel базовая включена в
  тариф.
- **Selectel / Timeweb / Aeza / Hostkey / VDSina** — все принимают
  оплату обычной российской картой и СБП без юрлица.

Источники наблюдений 2025-2026: LowEndTalk, Habr (антирейтинг VPS,
комментарии к оплате FirstVDS / VDSina), Otzovik (Timeweb), DTF (рейтинг
VPS апрель 2026), pxsl.ru / interfax.ru / cnews.ru (РКН и Hetzner).
