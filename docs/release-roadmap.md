...
7. **YooKassa shopId + «Мой налог»** (внерепно) — за неделю до запуска.
   Регистрация магазина в ЛК YooKassa, привязка к статусу самозанятого через
   приложение «Мой налог». Реальные shopId/secretKey подставляются в ENV
   billing-service. Без этого шага платежи остаются на mock-сервере.
